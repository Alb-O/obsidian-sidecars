...
export function isPathInFolderList(
	filePath: string,
...
export function isFileAllowedByFolderLists(
	filePath: string,
	settings: SidecarPluginSettings,
): boolean {
	// Use the correct property names from settings.ts
	const { blacklistFolders, whitelistFolders, useRegexForFolderLists } =
		settings;
	...
	if (whitelistFolders && whitelistFolders.length > 0) {
		if (!isPathInFolderList(filePath, whitelistFolders, useRegexForFolderLists))
			return false;
	}
	...
	if (blacklistFolders && blacklistFolders.length > 0) {
		if (isPathInFolderList(filePath, blacklistFolders, useRegexForFolderLists))
			return false;
	}
	return true;
}

export function isMonitoredFileUtil(
	filePath: string,
	settings: SidecarPluginSettings,
...
export function getSidecarPathUtil(
	sourcePath: string,
	settings: SidecarPluginSettings,
): string {
...
export function isSidecarFileUtil(
	filePath: string,
	settings: SidecarPluginSettings,
): boolean {
...
export function getSourcePathFromSidecarUtil(
	sidecarPath: string,
	settings: SidecarPluginSettings,
): string | null {
...
export function getRedirectPathUtil(
	sourcePath: string,
	settings: SidecarPluginSettings,
): string {
...
export function isRedirectFileUtil(
	filePath: string,
	settings: SidecarPluginSettings,
): boolean {
...
export function getSourcePathFromRedirectUtil(
	redirectPath: string,
	settings: SidecarPluginSettings,
): string | null {
