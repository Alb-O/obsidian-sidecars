...
export class ConfirmResetModal extends Modal {
	...
	onOpen() {
		...
		contentEl.createEl("p", { text: "Are you sure you want to reset all Sidecar settings to their default values? This cannot be undone." });
		const buttonRow = contentEl.createDiv('modal-button-container');
		new Setting(buttonRow)
			.addButton(btn => btn.setButtonText('Reset to defaults').setClass('mod-warning').onClick(() => {
				this.onAccept();
				this.close();
			}))
			.addButton(btn => btn.setButtonText('Cancel').setClass('mod-cancel').onClick(() => this.close()));
	}
}
